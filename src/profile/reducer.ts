...
import { IProfileAction, IProfileState, UPDATE_CONNECTED_PROFILE, UPDATE_PROFILE_LIST } from "./types";
import { defaultProfileState } from "./utils/defaultProfileState";

export function profile(state: IProfileState = defaultProfileState(), action: IProfileAction) : IProfileState{
    switch(action.type){
